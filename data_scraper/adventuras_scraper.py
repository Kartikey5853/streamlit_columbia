...
DATA_DIR = BASE_DIR / "adventuras_data"
...
CONFIG = {
    "BASE_URL": "https://www.adventuras.in",
    "COLLECTION_HANDLE": "adventuras-columbia",
    "OUTPUT_FILE": str(DATA_DIR / "adventuras_columbia_products.json"),
    "LOG_FILE": str(LOG_DIR / "adventuras_scraper.log"),
...
ADVENTURAS = MarketplaceConfig(
    name="adventuras",
    base_url="https://www.adventuras.in",
    search_urls=(
        "https://www.adventuras.in/search?q={query}&type=product",
        "https://www.adventuras.in/search?type=product&q={query}",
    ),
...
class AdventurasScraper(MarketplaceScraper):
    def __init__(self):
        super().__init__(ADVENTURAS)
...
def is_columbia_product(product):
    vendor = clean_text(product.get("vendor") or product.get("brand"))
    return vendor.casefold() == "columbia"
...
def material_from_text(text):
    ...
    patterns = [
        ...
        r"((?:rubber|synthetic|adapt\s+trax\S*)\s+outsole[^|.]{0,160})",
...
def product_from_shopify(product, detail_html=""):
    if not is_columbia_product(product):
        raise ValueError("Refusing to normalize a non-Columbia product")
...
def scrape_products_json(session, max_pages):
    for page in range(1, max_pages + 1):
        url = (
            f"{CONFIG['BASE_URL']}/collections/{CONFIG['COLLECTION_HANDLE']}/products.json"
            f"?limit={CONFIG['PRODUCTS_PER_PAGE']}&page={page}"
        )
        data = fetch(session, url, expect_json=True)
        fetched_products = data.get("products") or []
        products = [product for product in fetched_products if is_columbia_product(product)]
        stats["pages_visited"] += 1
        stats["products_seen"] += len(fetched_products)
        if not fetched_products:
            ...
            break
        rejected = len(fetched_products) - len(products)
        if rejected:
            log("Rejected non-Columbia collection products", page=page, rejected=rejected)

        added = 0
        for product in products:
            handle = product.get("handle") or ""
            detail_html = ""
            existing = products_by_id.get(product_key(product))
            needs_material = not existing or not existing.get("material_composition")
            if CONFIG["DETAIL_LOOKUP_FOR_MATERIAL"] and needs_material:
...
def discover_search_links(session):
    ...
    for query in SEARCH_QUERIES:
        url = f"{CONFIG['BASE_URL']}/pages/search?q={quote_plus(query)}"
        try:
        ...
        found = set(re.findall(r'href=["\'](?:https://www\.adventuras\.in)?/products/([^"\'?#]+)', html))
...
def scrape_discovered_handles(session, handles):
    for index, handle in enumerate(sorted(handles), start=1):
        ...
        try:
            ...
            if not is_columbia_product(product):
                log("Rejected non-Columbia discovered product", url=product_url(handle))
                continue
...
def parse_args():
    parser = argparse.ArgumentParser(description="Scrape Columbia products from the Adventuras Shopify store.")
    parser.add_argument("--max-pages", type=int, default=CONFIG["MAX_PRODUCT_PAGES"], help="Max collection JSON pages to scan")
